...
def test_staged_cancellation_restore_reseal_is_rejected_before_ledger_mutation() -> None:
    ...
    with pytest.raises(ValueError, match=r"correction restoration.*acceptance anchor"):
...
def test_published_cancellation_reseal_is_rejected_by_durable_core_anchor() -> None:
    ...
    assert (
        native_content_sha256(forged.body())
        != core._correction_cancellation_anchors[cancellation.request_fingerprint]
    )
    ...
    with pytest.raises(ValueError, match=r"cancellation.*acceptance anchor"):
        semantic_memory_state(core)
    with pytest.raises(ValueError, match=r"cancellation.*acceptance anchor"):
    ...
    assert _cancellation_runtime_shape(core) == before
    assert (
        core.application_receipts_for_feedback(cancellation.request.source_feedback_record_id)
        == receipts_before
    )
