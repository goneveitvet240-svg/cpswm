...
def main():
    ...
    def failed_house(c):
        previous_generation = context(c)["generation"]
        malformed = copy.deepcopy(house)
        check("house_has_objects", bool(malformed["objects"]))
        malformed["objects"][0]["assetId"] = "CPSWM_NONEXISTENT_ASSET_ADVERSARIAL"
        event = c.step(action="CreateHouse", house=malformed)
        check("house_failure_reported", not event.metadata["lastActionSuccess"])
        event = c.step(action="Pass")
        check(
            "failed_episode_stopped",
            not event.metadata["lastActionSuccess"]
            and "Episode discarded" in event.metadata["errorMessage"],
        )
        c.reset(scene=copy.deepcopy(house))
        fresh = context(c)
        check(
            "failed_episode_reset",
            fresh["generation"] != previous_generation and fresh["houseReady"],
        )
        roster(spawn(c, fresh["generation"]), 2)

    def held_object(c):
        info = context(c)
        objects = [o for o in c.last_event.metadata["objects"] if o.get("pickupable")]
        check("pickupable_fixture_exists", bool(objects))
        # Explicit adversarial boundary setup; forceAction is NOT a human-behaviour example.
        event = c.step(action="PickupObject", objectId=objects[0]["objectId"], forceAction=True)
        check(
            "actual_inventory_nonempty",
            event.metadata["lastActionSuccess"] and bool(event.metadata["inventoryObjects"]),
        )
        previous_inventory = copy.deepcopy(event.metadata["inventoryObjects"])
        reject_unchanged(
            c,
            action="CpswmSpawnAgents",
            generation=info["generation"],
            agentCount=2,
            requestId="held-object",
        )
        check(
            "inventory_not_cloned_or_lost",
            c.last_event.metadata["inventoryObjects"] == previous_inventory,
        )
        c.reset(scene=copy.deepcopy(house))
        roster(spawn(c, context(c)["generation"]), 2)

    def repeated_house(c):
        info = context(c)
        reject_unchanged(c, action="CreateHouse", house=copy.deepcopy(house))
        roster(spawn(c, info["generation"]), 2)
    ...
    if args.round == 2:
        cases = [
            ("continuous_state_machine", state_machine, True),
            ("partial_house_failure", failed_house, False),
            ("held_object_boundary", held_object, True),
            ("repeated_house", repeated_house, True),
        ]
    ...
    receipt = {
        ...
        "coverage": (
            "partial author runtime audit; partial-child failure and all collision states "
            "remain separate; forced pickup is negative boundary setup, not human evidence"
        ),
    }
