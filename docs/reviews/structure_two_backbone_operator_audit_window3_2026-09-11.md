...
> **2026-09-11 更正公告（第二轮）。** 本报告的下列结论已被
> [`structure_two_backbone_operator_audit_window3_round2_2026-09-11.md`](structure_two_backbone_operator_audit_window3_round2_2026-09-11.md)
> 收窄或撤回，请以第二轮报告为准：
>
> 1. §7 把 P0 修复写成「让冻结 P0 消费图成为真实数据依赖」。**收窄**：第一轮只做到了
>    「上游载荷作为参数传入并写入哈希」，没有任何语义检查。真正的依赖检查在第二轮实现。
> 2. §4 场景矩阵 S8 一行的「撤回后无残留、无重复计数」是对 **Hybrid 子账本**成立，
>    不是对全系统成立；该行直接调用了私有 `_hybrid_loop.retract_revision`，
>    不是正式反馈入口。第二轮用 `process_execution_feedback` 重建了全链路证据。
> 3. §5 的「算法实现等价」一列实际上只反映**绑定方式**。第二轮把两者拆开重建。
> 4. §8 的「历史 benchmark、全局 manifest、工程 checkpoint、P5 产物未失效（未触碰）」
>    **撤回**。第二轮按各自正式验证器实跑，给出四态失效登记。
> 5. §1 登记的四项 cryptography 环境失败，在项目原生环境（Python 3.13.5、
>    cryptography 47.0.0）下**不复现**；第二轮全部改用原生环境，不再使用语法降级夹具。
> 6. §6 G3 关于 CIAV 负观测「后验被计算后丢弃」的表述过粗。第二轮改为四层分解：
>    局部后验**确实**被计算并消费进该循环自己的分布；缺的是持久化与动作消费两层。
