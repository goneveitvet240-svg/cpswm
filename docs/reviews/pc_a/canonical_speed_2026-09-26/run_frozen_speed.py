"""Two frozen canonical-identity audits, then exact real old/new proposal scoring."""
...
def run(main, output):
    ...
    command(
        ...
        [
            ...
            "addopts=",
            "-q",
            str(AUDIT / "audit_canonical_second.py"),
    ...
    command(
        ...
        [
            python,
            str(AUDIT / "compare_real_scoring.py"),
            "--main",
            str(main),
            "--output",
            str(output / "real-equality"),
        ],
    )
    if sources() != baseline:
        raise ValueError("source changed across canonical equality run")
