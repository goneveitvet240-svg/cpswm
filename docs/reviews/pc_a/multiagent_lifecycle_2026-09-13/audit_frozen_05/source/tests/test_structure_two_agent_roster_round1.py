...
@pytest.mark.parametrize("coordinate", [float("nan"), float("inf"), True, "0", 10**1000])
...
def test_second_agent_initialization_failure_is_not_success():
    value = event()
    value.events[1].metadata["lastActionSuccess"] = False
    with pytest.raises(AgentRosterError):
        check(value)
