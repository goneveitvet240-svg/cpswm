...
实现状态：`s3-2_controlled_noise_baseline`
...
apps/evaluation_runner/prepare_direction_three_findingdory.py
  FindingDory JSONL metadata dry-run 与质量报告入口
```
...
- S3-2 扩展矩阵覆盖 7 臂 × 3 严重度 × 3 种子 × 6 探针 = 378 cases，输出
  Student-t 95% 多种子诊断区间和 scenario-level strata（情境级分层）；
- 统一指标覆盖 Recall@k、MRR、unknown AUROC/AUPRC、selective risk/coverage、
  ECE/Brier/NLL、误拿、询问、identity switch、任务成功、恢复和五类成本；
- FindingDory 官方八列 metadata adapter 明确保留答案帧与 `[[-1]]` 无答案语义，
  并将实例、人物、事件、位置、pose、可见性和候选真值标为不可用，不做伪造；
- prediction cache 按 split、visible episode hash、model version、calibration domain 和
  inference config 绑定；跨 split、真值字段、版本漂移与内容篡改均 fail closed；
- 结构三定向测试共 129 项。
...
- object-instance Recall@k、MRR、Top-k accuracy；
...
| S3-1 | oracle | 进行中：已完成首个闭环、17 情境真值套件及 3 个多维组合 probe；仍需非搜索失败后的多步恢复及完整真实路径指标 |
| S3-2 | controlled-noise | 进行中：已完成 60-case 基线与 378-case 严重度／多种子／组合扩展；观察选择偏差估计和更大种子数稳定区间仍待补齐 |
| S3-3 | perception_baseline | 边界已搭建：FindingDory 八列 metadata adapter 与 prediction cache 已完成；真实 VLM、RGB-D、点云和实例重识别离线预测尚未接入 |
...
当前 S3-2 结果只验证“噪声注入和评价管线可工作”，不证明所用噪声强度代表真实分布。`retrieval_miss_open_set` 通过显式放大 unknown likelihood 构造压力测试，其 1.0 top-1 只是合成 sanity check（健全性检查），不是 unknown 方法成绩。

仍需补齐：

- observation selection bias（观察选择偏差）的估计目标、logging policy（日志策略）与校正方法；该项已登记为用户后续决策门，当前先保留原始选择概率字段；
- 将当前 3-seed Student-t 诊断区间扩展为足够种子数的稳定区间；
- candidate-set size、risk level、人物数、遮挡与时间跨度分层；
- FindingDory Habitat 原始 episode schema、scene/time grouping 与许可边界核验；官方
  SFT 八列 metadata adapter 已完成，但不足以构造完整结构三 episode；
