...
class FixtureController:
    ...
    def sync(self, sequence):
        ...
        self.server.raw_metadata = {
            "sequenceId": sequence,
            "activeAgentId": self.last_event.metadata["agentId"],
...
@pytest.mark.parametrize("mode", ["absent", "duplicate", "bool_sequence", "wrong_pose", "wrong_sequence"])
def test_forged_initial_wire_rejected(mode):
    controller = FixtureController()
    raw = controller.server.raw_metadata
    if mode == "absent":
        controller.server.raw_metadata = None
    elif mode == "duplicate":
        raw["agents"][1]["agentId"] = 0
    elif mode == "bool_sequence":
        raw["sequenceId"] = True
    elif mode == "wrong_pose":
        raw["agents"][1]["agent"]["position"]["x"] = 99
    else:
        controller.server.sequence_id += 1
    with pytest.raises(AgentRosterError):
        session(controller)
    assert controller.calls == []


def test_external_sequence_kwarg_never_sent():
    controller = FixtureController()
    with pytest.raises(ValueError):
        session(controller).step(action="Pass", agentId=0, sequenceId=0)
    assert controller.calls == []


def test_rejected_address_does_not_poison_legal_session():
    controller = FixtureController()
    checked = session(controller)
    with pytest.raises(ValueError):
        checked.step(action="Pass", agentId=99)
    assert checked.step(action="Pass", agentId=1).metadata["agentId"] == 1
