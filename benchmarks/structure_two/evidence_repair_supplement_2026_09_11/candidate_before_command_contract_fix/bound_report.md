...
本轮修复源码提交为 `3dce0485234f941fc9520e52b1fe9d8f14ded892`。
...
- P0 当前消费者仍使用 `benchmarks/p0_checkpoint/content_manifest_v0_3.json`；旧版原字节另存
  ...
  同一目录还保留 `reviewed_v0_5_compatibility_audit.json`，当前 v0.5 兼容性审计另按新源码再生。
- 原 v0.2 五类、11 份历史、旧历史报告、旧 checkpoint 与回执逐字节核对已审查提交，完整列表见
  `preserved_reviewed_evidence.json`。它们仍可作为对应历史源码版本证据，不能手改来源摘要升级为当前。
- 普通库进程不再自动获得正式来源身份；消费者应启动目标工作树中的正式 CLI，或在任何项目
  依赖导入前显式使用标准库来源启动器。不能用原工作树加载的代码加另一个 `--repository-root`
  赋予目标工作树身份。
...
原始反例日志末尾的空行曾触发 Git 空白检查，已改为 gzip 无损归档；日志字节没有修剪，
对应失败检查和最终重跑分别记录。
...
五类 v0.3 已于 2026-09-11 13:22:14–13:52:10 UTC 完成真实生成和发布前完整重算，
整批退出 0（1795.6 秒）。安全发布器先冻结 JSON 值再调用真实验证器；验证器强制 fresh，
完成后才发布，因此不是只改外层摘要或快速检查。存盘后的完整再核验由原生审计矩阵执行。
`preserved_and_new_before_p0` 再次核对 19 份已审查工件及旧 P0 快照字节，且五类结果排除根级
`source_binding`、`evidence_context`、`content_sha256` 后全部字段与 v0.2 相等。
真实聚合 CLI 再次生成已封存路径被 `FileExistsError` 拒绝（exit 1），作为预期拒绝记录，
不将其错误计入正常生成成功。

两个历史入口于 13:54:30–13:59:22 UTC 分别完成完整执行并退出 0，各报告 11 份 Git 字节、
4 份来源可恢复、1 次指定失败重算。`4103bea` 全工件相等；最早版本仍记录导入失败。
v0.5 兼容性审计真实再生，当前文件数为 311，与历史冻结 v0.5 仍不兼容，未改成兼容。
随后 P0 原生生成器产生 manifest SHA-256：
`5423f71794251b625541f9ca7011a39a1e3e385d165457c8d292d8e562e9fb95`。

本轮原生工程矩阵于 14:00:11–14:49:50 UTC 完成，10 条命令全部 exit 0。
核心套件实际 **3908 passed、1 skipped、1 xfailed、0 failed、0 errors**，共 3910 项，
耗时 1979.99 秒。P0 对抗矩阵 **121 passed**；mypy **310** 个源码文件无错误；
Ruff check/format、compileall、冻结离线环境核验和 Git 差异检查均通过，format 覆盖 663 个文件。
存盘五类重算和历史 `--verify` 是回执内的真实执行，分别 exit 0，未关闭重算。

审计回执 content SHA-256：
`09a0b775ef8ca752e5ae9ce8f65bdaaa16516240216bf67a0479e2a4f49f0df4`。
执行前后 P0 摘要相等，环境指纹也相等；命令各自绑定本工作树实际 cwd、原生执行文件、环境
覆盖和 stdout/stderr，未借用其他窗口回执。

原生核心的 1 个 skip 来自外层环境不允许嵌套 macOS sandbox-exec。按原断言、同一工作树
与 Python 环境，取得本机执行权限后补跑
`test_real_sandbox_ten_arm_receipts_bind_full_support_and_deny_opening_bytes`，实际通过。
该用例使用既有合成支持样本，不是新确认集，协议失效/不得授权边界仍被断言。
同次补跑原有 RLS 消融用例得到 **1 passed、1 xfailed**，没有 skipped 或 failed。
...
均完成后才封存交付。不存在等待用户选择的科学改动，已知历史和安全边界按上文保留。
...
已封存 v0.3 也不可覆盖，整合后的当前结果应使用新的证据版本或内容寻址目录，并调整
`CURRENT_DIRECTORY`、五个默认输出、证据版本及检查点消费者引用。任何被绑定输入变化都必须重验受影响层。本分支通过不能自动继承，不能把三个分支各自的绿色
结果拼成整体通过。
