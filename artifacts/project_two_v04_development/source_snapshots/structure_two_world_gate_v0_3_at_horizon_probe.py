...
def _ranked(scores: dict[str, float], locations: tuple[str, ...], first: str | None) -> list[str]:
    order = sorted(locations, key=lambda item: (-scores.get(item, 0.0), item))
...
class _CountingEstimator:
    ...
    def mode(self, context: str) -> str:
        return _argmax(self.scores(context))
...
def evaluate_rollout_v0_3(
    ...
    *,
    ...
    """Score the frozen v0.2 rule, the witness, and every reachability reference.
    ...
    for step in rollout.steps:
        ...
        frozen_mode = _argmax(frozen_scores)
        ...
        orders = {
            ...
            "frozen": _ranked(frozen_scores, locations, head or frozen_mode),
        ...
        oracle_head = head or _argmax(posterior)
        ...
        errors["search_uncond"] += (head or _argmax(uncond)) != step.true_location
        errors["search_actor"] += (head or _argmax(actor_posterior)) != step.true_location
