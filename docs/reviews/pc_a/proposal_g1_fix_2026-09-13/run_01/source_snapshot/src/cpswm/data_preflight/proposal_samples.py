...
class ProposalSample(ContractModel):
    ...
    def validate_sample(self) -> Self:
        ...
        for binding in self.location_support:
            ...
            if (
                binding.origin == "snapshot"
                and binding.source_snapshot_id != self.source_snapshot_id
            ):
                raise ValueError("location originates from a different snapshot")
            if binding.origin == "visible_evidence":
                record = detections.get(binding.source_record_id)
                if (
