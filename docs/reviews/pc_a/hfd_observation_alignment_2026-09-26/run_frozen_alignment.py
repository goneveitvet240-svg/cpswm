"""Freeze -> two sequential audits -> source reconstruction -> real RGB frontend."""
...
def run(main, output):
    ...
    command(
        "round2",
        [python, "-m", "pytest", "-o", "addopts=", "-q", str(AUDIT / "audit_alignment_second.py")],
    )
    ...
    command(
        ...
        [
            python,
            "tools/run_hfd_pixel_frontend.py",
            "--runtime",
            str(runtime),
            "--manifest-sha256",
            pin,
