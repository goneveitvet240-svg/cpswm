...
实现状态：`s3-1_oracle_closed_loop_baseline`
...
| S3-1 | oracle | 进行中：已完成首个 M29-L0 真值闭环基线；仍需扩展多情境身份/几何/事件 oracle 轨道与成套评价 |
| S3-2 | controlled-noise | 六通道独立注入噪声、缺失、相关性和误校准 |
| S3-3 | perception_baseline | 接入真实 VLM、RGB-D、点云和实例重识别离线数据 |
...
本轮没有缩小结构三的完整范围，也没有把单个方法提前指定为唯一论文路线。已完成的是首个可回放 oracle closed loop（真值闭环）基线：真值查询编译和候选召回、信息价值观察选择、使用同一似然模型的观测更新、执行反馈规范写回，以及失败后的候选再规划。

它尚不等于 S3-1 全部完成。下一轮仍需在保留六项完整能力的前提下，补齐：

- 多人物、相似实例、隐藏事件、容器和跨时间移动的 oracle scenario suite（真值情境套件）；
- 身份、位置、事件、人物和习惯阶段分别可审计的真值轨道；
- 询问、多视角、开容器、触觉、停止和拒答的完整 oracle 动作覆盖；
- 未找到、抓取失败、滑落、部分成功和未知结果的多步恢复轨迹；
- 与结构三评价协议对应的实例检索、误拿率、任务成功率、路径/时间/打扰成本报告。
