...
def require_agent_roster(
    ...
    *,
    ...
    for item in events:
        ...
        if action in {"Initialize", "CreateHouse"} and (
            metadata.get("lastActionSuccess") is not True
            or metadata.get("errorMessage") not in (None, "")
            or (action == "Initialize" and metadata.get("lastAction") != "Initialize")
        ):
            raise AgentRosterError("one or more agents did not initialize successfully")
        ...
        for key in ("position", "rotation"):
            ...
            try:
                valid = all(type(v) in (int, float) and math.isfinite(v) for v in point.values())
...
class VerifiedAgentSession:
    ...
    def _wire_snapshot(self, event: Any) -> dict[str, Any]:
        ...
        if (
            ...
            or sequence < 0
            or type(getattr(server, "sequence_id", None)) is not int
