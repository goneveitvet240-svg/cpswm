...
class DeferredCorrectionCancellation:
    ...
    def validate_content(self) -> None:
        ...
        if (
            ...
            or any(
                event.revision_id not in restore_by_revision
                for event in self.restore_fast_events
            )
...
class CorePrototypeSpine:
    ...
    def _validate_correction_state_anchors(self) -> None:
        ...
        if staged != self._deferred_correction_restore_anchors:
            raise ValueError(
                "deferred correction restoration differs from core acceptance anchors"
            )
    ...
    def _capture_revision_transaction(
        ...
        checkpoint: dict[str, object] = {
            ...
            "deferred_correction_restore_anchors": dict(
                self._deferred_correction_restore_anchors
            ),
            "correction_cancellation_anchors": dict(
                self._correction_cancellation_anchors
            ),
    ...
    def _restore_revision_transaction(self, checkpoint: Mapping[str, object]) -> None:
        ...
        self._particle_input_anchors = cast(
            dict[UUID, str], checkpoint["particle_input_anchors"]
        )
