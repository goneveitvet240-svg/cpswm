...
方向结构三的独立定义、契约和可运行基线见 `docs/结构三/项目方向结构三_模糊语言联合后验主动确认与具身闭环_v1.0.md`。当前已推进到 `s3-2_controlled_noise_baseline`：17 个确定性真值情境已覆盖五维独立真值、主动观察、停止/拒答、六类执行结果和 `not_found` 再规划；统一 episode contract 会隔离评价器真值、记录 observation selection probability（观察选择概率）并阻断跨 split 泄漏；首个 S3-2 矩阵以 10 个噪声条件运行 60 cases。FindingDory 官方八列 metadata adapter（元数据适配器）和 split-bound prediction cache（划分绑定预测缓存）已经落地，但官方 SFT 元数据不足以构造实例轨迹、人物／事件／习惯真值或完整结构三 episode。它仍不代表 S3-1/S3-2 全部完成，也不代表真实 VIO、VLM、触觉、MPC 或机器人硬件已经验证。详见 `docs/experiments/direction_three_s3_1_oracle_suite_v0_1.md`、`docs/experiments/direction_three_s3_2_controlled_noise_v0_1.md` 与 `docs/experiments/direction_three_findingdory_metadata_audit_v0_1.md`。
