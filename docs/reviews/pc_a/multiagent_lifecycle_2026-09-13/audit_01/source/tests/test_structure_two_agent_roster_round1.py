...
def event(count=2, active=0, action="Initialize"):
    items = [SimpleNamespace(metadata={
        "agentId": i, "sceneName": "FloorPlan1", "lastAction": action,
        "lastActionSuccess": True, "errorMessage": "",
        "agent": {"position": {"x": float(i), "y": 1.0, "z": 0.0},
...
def test_legal_rosters():
    for count in range(1, 7):
        assert require_agent_roster(event(count), count=count, action="Initialize", active_id=0) == tuple(range(count))
...
@pytest.mark.parametrize("field,replacement", [
    ("lastActionSuccess", False), ("lastActionSuccess", 1),
    ("lastAction", "CreateHouse"), ("errorMessage", "NotImplemented"),
    ("sceneName", ""), ("sceneName", None), ("agent", None),
])
