...
def validate_sensor_bytes(raw: bytes, depth_unit: str | None) -> None:
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(raw)) as archive:
            ...
            # Validate shape/byte count before NumPy can allocate from an untrusted header.
            for info in archive.infolist():
                with archive.open(info) as handle:
                    ...
                    if version == (1, 0):
                        ...
                        raise ValueError("unsupported sensor array format")
                    if dtype.hasobject or any(type(n) is not int or n < 1 for n in shape):
                        raise ValueError("unsafe sensor array dtype or shape")
                    if math.prod(shape) * dtype.itemsize != info.file_size - handle.tell():
                        raise ValueError("sensor array shape does not match payload bytes")
...
def read_sensor_snapshot(path: Path, *, expected_sha256: str, depth_unit: str | None) -> bytes:
    ...
    if depth_unit is not None and (type(depth_unit) is not str or depth_unit != "m"):
        raise ValueError("only declared meter depth or absent depth supported")
    if (
